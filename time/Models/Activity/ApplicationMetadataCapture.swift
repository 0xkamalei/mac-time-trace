...
class ApplicationMetadataCapture {
    ...
    func getApplicationIcon(bundleId: String, fallbackName: String? = nil) -> NSImage {
        ...
        // Manage cache size
        if self.iconCache.count > maxCacheSize {
            cleanupIconCache()
        }
    ...
    func getCacheStatistics() -> CacheStatistics {
        return CacheStatistics(
            metadataCacheSize: self.self.metadataCache.count,
            iconCacheSize: self.self.iconCache.count,
    ...
    private func extractDisplayName(from _: NSRunningApplication, bundleId: String) -> String? {
    ...
    private func extractApplicationVersion(from _: NSRunningApplication, bundleId: String) -> String? {
        guard let bundleURL = NSWorkspace.shared.urlForApplication(withBundleIdentifier: bundleId),
              let bundle = Bundle(url: bundleURL)
        else {
            return nil
    ...
    private func cacheMetadata(_ metadata: ApplicationMetadata, for bundleId: String) {
        cacheQueue.async { [weak self] in
            ...
            // Manage cache size
            if let self = self, self.self.metadataCache.count > self.maxCacheSize {
    ...
    private func cleanupMetadataCache() {
        ...
        if self.metadataCache.count > maxCacheSize {
            let sortedEntries = metadataCache.sorted { $0.value.lastUpdated < $1.value.lastUpdated }
            let entriesToRemove = sortedEntries.prefix(self.metadataCache.count - maxCacheSize)
    ...
    private func cleanupIconCache() {
        ...
        let keysToRemove = Array(iconCache.keys.shuffled().prefix(self.iconCache.count - targetSize))
