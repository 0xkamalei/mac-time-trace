...
class BackgroundProcessingManager: ObservableObject {
    ...
    @Published private(set) var processingMetrics: ProcessingMetrics = .init()
    ...
    private var operationThrottler: OperationThrottler = .init()
    private var rateLimiter: RateLimiter = .init()

    // Performance monitoring
    private var performanceTracker: BackgroundPerformanceTracker = .init()
    ...
    func executeHeavyOperation<T>(
        ...
        progressCallback _: ((Double) -> Void)? = nil
    ) async throws -> T {
        ...
        for batch in batches {
            ...
            let batchResults = try await withThrowingTaskGroup(of: T.self) { group in
                for operation in batch {
                    group.addTask {
                        try await self.executeBackgroundOperation(operation)
                    }
                }
...
class RateLimiter {
    ...
    private var lastOperationTime: Date = .init()
    private let lock = NSLock()
