...
class SearchPerformanceOptimizer {
    ...
    // Performance metrics
    private var searchMetrics: SearchMetrics = .init()
    ...
    func optimizeQuery(_ query: String) -> String {
        ...
        let filteredTerms = terms.filter { term in
            ...
            term.count >= 2 || term.contains(":") || term.hasPrefix("-") || term.hasPrefix("\"")
        }
...
struct SearchMetrics {
    ...
    mutating func recordSearch(query: String, resultCount _: Int, searchTime: TimeInterval, cacheHit: Bool) {
        totalSearches += 1
