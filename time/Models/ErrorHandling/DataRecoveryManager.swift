...
class DataRecoveryManager: ObservableObject {
    ...
    private func recoverIncompleteOperation(_: IncompleteOperation) async throws {
    ...
    private func recoverSession(_: CorruptedSession) async throws {
        ...
        Timer.scheduledTimer(withTimeInterval: 3600, repeats: true) { _ in // Every hour
            Task { @MainActor in
                if self.shouldCreateAutomaticBackup() {
                    _ = await self.createBackup(type: .automatic)
                }
            }
        }
    }
